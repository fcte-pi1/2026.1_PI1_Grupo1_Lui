...
export function RobotMap({ telemetria, celulasExploradas, trajetoRapido = [] }: PropriedadesMapa) {
  ...
  return (
    <div className="relative w-full h-full flex-1">
      <div 
        className="absolute inset-4 md:inset-8 m-auto w-full max-w-full max-h-full aspect-square grid bg-[#0f172a] rounded-xl shadow-[inset_0_0_30px_rgba(0,0,0,0.6)] border border-slate-700/60 overflow-hidden"
        style={{ 
      ...
      >
        ...
        {Array.from({ length: tamanho * tamanho }).map((_, index) => {
          ...
          let classesParedes = "border border-slate-800/30 border-solid"; 
          let corFundo = "bg-transparent";

          if (celula) {
            corFundo = "bg-[#1e293b]/40"; 
            classesParedes = `
              border-solid
              ${celula.paredes.norte ? 'border-t-[3px] border-t-slate-200' : 'border-t border-t-slate-800/30'}
              ${celula.paredes.sul ? 'border-b-[3px] border-b-slate-200' : 'border-b border-b-slate-800/30'}
              ${celula.paredes.leste ? 'border-r-[3px] border-r-slate-200' : 'border-r border-r-slate-800/30'}
              ${celula.paredes.oeste ? 'border-l-[3px] border-l-slate-200' : 'border-l border-l-slate-800/30'}
            `;
          }
